...
VALUES ('a-article', 'A Article',
        ...
        ( SELECT id FROM users WHERE username = 'member1' ), 'member1', TRUE, NOW(), 'A Content This is a sample article content for A Article. Point 1 Point 2 Point 3 Enjoy reading!'),
       ('b-article', 'B Article',
        E'### B Content\n\nThis is a sample article content for B Article.\n\n- Point A\n- Point B\n- Point C\n\nHappy coding!',
        ( SELECT id FROM users WHERE username = 'member1' ), 'member1', TRUE, NOW(), 'B Content This is a sample article content for B Article. Point A Point B Point C Happy coding!'),
       ('c-article', 'C Article',
        E'### C Content\n\nThis is a sample article content for C Article.\n\n- Item X\n- Item Y\n- Item Z\n\nKeep learning!',
        ( SELECT id FROM users WHERE username = 'member2' ), 'member2', TRUE, NOW(), 'C Content This is a sample article content for C Article. Item X Item Y Item Z Keep learning!');
...
INSERT INTO public.series_chapters(title, content, sequence, series_id, writer_id, published, published_at, normalized_content)
VALUES ('Chapter 1 of A Series', 'Content for Chapter 1 of A Series.', 1,
        ( SELECT id FROM series WHERE slug = 'a-series' ), ( SELECT id FROM users WHERE username = 'admin' ), TRUE, NOW(), 'Chapter 1 of A Series Content for Chapter 1 of A Series.'),
       ('Chapter 2 of A Series', 'Content for Chapter 2 of A Series.', 2,
        ( SELECT id FROM series WHERE slug = 'a-series' ), ( SELECT id FROM users WHERE username = 'admin' ), TRUE, NOW(), 'Chapter 2 of A Series Content for Chapter 2 of A Series.'),
       ('Chapter 1 of B Series', 'Content for Chapter 1 of B Series.', 1,
        ( SELECT id FROM series WHERE slug = 'b-series' ), ( SELECT id FROM users WHERE username = 'member1' ), TRUE, NOW(), 'Chapter 1 of B Series Content for Chapter 1 of B Series.');
...
VALUES ('a-question', 'This is the content of question A.',
        ...
        ( SELECT id FROM users WHERE username = 'member1' ), 'member1',
        NOW() - INTERVAL '1 hour', NOW() - INTERVAL '1 hour', 'Question A Content I have a question about topic A. Can someone help? Thanks in advance!'),
       ('b-question', 'This is the content of question B.',
        E'# Question B Content\n\nI am facing an issue with topic B. Any advice?\n\nAppreciate your help!',
        ( SELECT id FROM users WHERE username = 'member1' ), 'member1',
       NOW() - INTERVAL '30 minutes', NOW() - INTERVAL '30 minutes', 'Question B Content I am facing an issue with topic B. Any advice? Appreciate your help!'),
       ('c-question', 'This is the content of question C.',
        ...
        NOW(), NOW(), 'Question C Content Can anyone explain topic C to me? Looking forward to your insights!');
INSERT INTO public.questions (slug, title, content, writer_id, writer_username, created_at, updated_at, normalized_content)
VALUES ('javascript-closure-실용적인-예제-질문', '자바스크립트 클로저(Closure)의 실용적인 예제에 대해 질문합니다.',
        ...
        ( SELECT id FROM users WHERE username = 'member1' ), 'member1',
        NOW() - INTERVAL '6 days 3 hours 15 minutes', NOW() - INTERVAL '6 days 3 hours 15 minutes', '자바스크립트 클로저, 언제 어떻게 써야 할까요? 안녕하세요, 자바스크립트를 공부하는 중급 개발자입니다. 클로저(Closure)라는 개념이 "함수가 선언된 렉시컬 환경을 기억하는 함수"라는 것은 이론적으로 이해했습니다. 내부 함수가 외부 함수의 변수에 접근할 수 있다는 기본적인 예제는 수없이 봤습니다. function outer() { const outerVar = ''외부 변수''; function inner() { console.log(outerVar); // 외부 함수의 변수에 접근 } return inner; } const innerFunc = outer(); innerFunc(); // "외부 변수" 출력 하지만 이런 교과서적인 예제를 넘어서, 실제 프로젝트에서 클로저가 "왜 필요하고, 어떻게 유용하게 사용되는지"에 대한 감이 잘 오지 않습니다. 제가 궁금한 점은 다음과 같습니다. 데이터 은닉(Data Encapsulation): 클로저를 사용해 private 변수처럼 흉내 낼 수 있다고 들었습니다. class 문법이 있는데도 굳이 클로저를 사용해 데이터를 은닉하는 경우가 있나요? 있다면 어떤 장점이 있을까요? 이벤트 핸들러와 반복문: 반복문 안에서 이벤트 리스너를 추가할 때 클로저가 잘못 사용되면 발생하는 문제와 해결책은 익히 알고 있습니다. 이것 외에 이벤트 핸들링에서 클로저가 유용하게 쓰이는 다른 패턴이 있을까요? 성능과 메모리: 클로저를 사용하면 외부 함수의 스코프가 메모리에 계속 남아있게 되는데, 이로 인한 메모리 누수(memory leak)의 위험은 없나요? 실제 프로젝트에서 클로저를 사용할 때 메모리 관점에서 특별히 주의해야 할 점이 있다면 알려주세요. 실무에서 클로저를 유용하게 사용하셨던 경험이나 좋은 패턴이 있다면 코드 예제와 함께 공유해주시면 정말 감사하겠습니다!');
