...
(
    ...
    path            text GENERATED ALWAYS AS ('articles/@' || writer_username || '/' || slug) STORED,
    title           text         NOT NULL,
...
(
    id              bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    slug            varchar(50)          NOT NULL,
    path            text GENERATED ALWAYS AS ('questions/@' || writer_username || '/' || slug) STORED,
    title           text                 NOT NULL,
    content         text                 NOT NULL,
...
CREATE INDEX IF NOT EXISTS answers_question_id_created_idx ON public.answers (question_id, created_at ASC);

DO 'BEGIN IF NOT EXISTS ( SELECT 1 FROM pg_type WHERE typname = ''star_target_enum'' ) THEN CREATE TYPE star_target_enum AS ENUM (''ARTICLE'',''BOOKS'',''QUESTION''); END IF; END' LANGUAGE plpgsql;
...
SELECT path, title, NULL::question_status_enum AS question_status, updated_at, writer_id
FROM articles
UNION ALL
SELECT path, title, status AS question_status, updated_at, writer_id
FROM questions;
CREATE INDEX IF NOT EXISTS articles_writer_updated_idx ON articles (writer_id, updated_at DESC) INCLUDE (path, title);
CREATE INDEX IF NOT EXISTS questions_writer_updated_idx ON questions (writer_id, updated_at DESC) INCLUDE (path, title, status);
