...
(
    ...
    created_at timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    expired_at timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP + INTERVAL '30 day',
    user_id    bigint       NOT NULL
        CONSTRAINT refresh_tokens_user_id_fk REFERENCES public.users ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS refresh_tokens_hash_idx ON refresh_tokens USING HASH (token);
SELECT cron.schedule('refresh_token-cleanup', '30 seconds',
                     $$DELETE FROM public.refresh_tokens WHERE expired_at < CURRENT_TIMESTAMP$$);
