...
CREATE INDEX IF NOT EXISTS articles_fts_idx ON public.articles USING pgroonga ((ARRAY [title, normalized_content, topics_flat])) WITH (tokenizer='TokenMecab', normalizer='NormalizerAuto');
...
CREATE INDEX IF NOT EXISTS articles_writer_username_published_at_idx ON public.articles (writer_username, published_at);
...
CREATE INDEX IF NOT EXISTS questions_fts_idx ON public.questions USING pgroonga ((ARRAY [title, normalized_content, topics_flat])) WITH (tokenizer='TokenMecab', normalizer='NormalizerAuto');
